...
// function OptionGroup({ group, selections, onChange }) {
//   // 🧠 normalize items (supports both structured + raw string format)
//   const items = useMemo(() => {
//     const raw = group?.items || [];
...
//       // CASE 2: raw string format -> "Thin Crust, Normal"
//       if (typeof item === "string") {
//         const parts = item.split(",").map(s => s.trim());
...
//           onClick={e => e.stopPropagation()}
//           className="bg-card w-full sm:max-w-md rounded-t-3xl sm:rounded-2xl overflow-hidden flex flex-col max-h-[92vh] relative"
//         >
//           {/* Close */}
//           <button
//             onClick={onClose}
//             className="absolute top-4 right-4 z-10 h-8 w-8 rounded-full bg-black/40 text-white flex items-center justify-center backdrop-blur-sm hover:bg-black/60 transition-colors"
//           >
//             <X className="h-4 w-4" />
//           </button>

//           {/* Image */}
//           <div className="relative w-full  flex items-center bg-transparent justify-center flex-shrink-0" style={{ minHeight: 200, maxHeight: 240 }}>
//             {product.image ? (
//               <img src={product.image} alt={getLocalizedField(product, 'name')} className="w-full object-contain" style={{ maxHeight: 240 }} />
//             ) : (
//               <img
//                 src={noImage}
//                 alt="no-image"
...
//           {/* Scrollable content */}
//           <div className="flex-1 overflow-y-auto p-5 space-y-5 bg-muted">
//             {/* Header info */}
//             <div>
//               {/* NAME */}
...
//               onChange={e => setNote(e.target.value)}
//               rows={2}
//               className="w-full rounded-xl border border-input bg-background px-3 py-2.5 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-primary/30 placeholder:text-muted-foreground"
//             />
//           </div>

//           {/* Bottom bar */}
//           <div className="p-4 border-t border-border/50 flex items-center gap-3 flex-shrink-0 bg-card">
//             <div className="flex items-center gap-2.5 bg-secondary rounded-full px-3 py-1.5">
//               <button onClick={() => setQuantity(q => Math.max(1, q - 1))} className="h-6 w-6 rounded-full flex items-center justify-center hover:bg-background/60 transition-colors">
//                 <Minus className="h-3.5 w-3.5" />
//               </button>
//               <span className="text-sm font-bold w-5 text-center">{quantity}</span>
//               <button onClick={() => setQuantity(q => q + 1)} className="h-6 w-6 rounded-full flex items-center justify-center hover:bg-background/60 transition-colors">
//                 <Plus className="h-3.5 w-3.5" />
//               </button>
//             </div>
...
//               onClick={handleAdd}
//               disabled={!canAdd}
//               className="flex-1 rounded-xl h-11 bg-primary hover:bg-primary/90 font-semibold text-sm disabled:opacity-50 disabled:cursor-not-allowed"
//             >
//               {t('addToCart')} — ${totalPrice.toFixed(2)}
//             </Button>
//           </div>
