  ...
  return (
    <div className="sticky top-16 z-40 bg-background/95 backdrop-blur-xl border-border/30">
      <div className="max-w-5xl mx-auto px-4 py-2.5 relative">
        <nav
          ref={navRef}
          className="flex gap-2 overflow-x-auto pb-0.5 no-scrollbar" // Added no-scrollbar for cleaner look
        >
          {items.map(item => (
            <button
              key={item.id}
              onClick={() => handleCategoryClick(item.id)} // ✅ Updated this line
              className={`relative flex items-center gap-1.5 px-3.5 py-1.5 font-serif rounded-full text-sm font-medium whitespace-nowrap transition-all duration-200
